...
import {createPost, getMorePosts, getPosts} from "../firestore/posts";
import {addPost, removeAllPosts} from "../states/reducer/postsSlice";

export default function ModalCreatePost({onClose, isOpenState, closeTrigger}) {
    ...
    function postNow() {
        ...
        createPost(payloadData).then(res => {
            console.log("result -->", {res, payloadData});
        }).catch(err => {
    ...
    return <>
        <Dialog as="div" className="fixed inset-0 z-10 overflow-hidden" onClose={onClose} open={isOpenState}>
